...
extern "C" void kernel_main(uint64_t multiboot_magic, uint64_t multiboot_info_ptr) {
    ...
    // 3. WŁĄCZENIE EKRANU W TRYBIE GRAFICZNYM
    ...
    // 5. Inicjalizacja asynchronicznych mechanizmów wejścia
    inicjalizuj_apic();
    ...
    // 7. Konfiguracja RAM-Dysku jako superszybkiego bufora (Cache) pod System Plików
    ...
    // TERAZ dysk twardy i matryca ekranu są gotowe, więc inicjalizujemy System Plików!
    inicjalizuj_psf((void*)adres_wirtualny_dysku, rozmiar_dysku);

    // Tworzenie zaufanego drzewa katalogów
    ...
    // 9. Odblokowanie przerwań sprzętowych
    asm volatile("sti");

    // Zapisujemy wyodrębnioną binarkę powłoki na wirtualny RAM Dysk (Hybrydowy AHCI)
    usun_twor("/shell.bur"); 
    ...
    bws_uruchom_program_z_pliku("/shell.bur", 4, 0xFFFFFFFF);

    // 10. Pętla bezczynności - Kernel czeka na eventy (Halt state)
    while (true) {
        asm volatile ("hlt");
    }
}